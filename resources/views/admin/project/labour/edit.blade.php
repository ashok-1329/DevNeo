...
@section('content')
    <div class="card-container">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4 bg-secondary rounded px-3 py-2">
            <div>
                <h4 class="mb-1 text-uppercase ls-1 text-light">Edit Labour</h4>
                <nav class="small text-light">
                    ...
                    <span class="mx-1">/</span>
                    <span>Edit Labour</span>
                </nav>
            </div>
            <div class="d-flex gap-2">
                {{-- <a href="{{ route('admin.project.labour.show', $labour->id) }}" class="btn btn-secondary btn-sm">
        ...
        <form action="{{ route('admin.project.labour.update', $labour->id) }}" method="POST" id="labourForm" novalidate>
            @csrf
            @method('PUT')
                        ...
                        <select name="region" id="regionSelect"
                            ...
                            @foreach ($project_regions as $region)
                                <option value="{{ $region->id }}"
                                    {{ old('region', $labour->region_id) == $region->id ? 'selected' : '' }}>
                                    {{ $region->name }}
                                </option>
                            @endforeach
                        </select>
                        <input type="text" name="region_name" id="regionNameInput"
                        ...
                        <select name="employment_type" id="employmentType"
                            ...
                            @foreach ($employment_types as $type)
                                <option value="{{ $type->id }}"
                                    {{ old('employment_type', $labour->employment_type) == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="employer_position" id="employerPosition"
                            ...
                            @foreach ($labour_positions as $position)
                                <option value="{{ $position->id }}"
                                    {{ old('employer_position', $labour->position) == $position->id ? 'selected' : '' }}>
                                    {{ $position->name }}
                                </option>
                            @endforeach
                        </select>
                        <div class="invalid-feedback">
                            @error('employer_position') {{ $message }} @enderror
                        </div>
                    </div>
                        ...
                        <select name="employer_supplier" id="employerSupplier"
                            ...
                            @foreach ($suppliers as $supplier)
                                <option value="{{ $supplier->id }}"
                                    {{ old('employer_supplier', $labour->employer_supplier) == $supplier->id ? 'selected' : '' }}>
                                    {{ $supplier->name }}
                                </option>
                            @endforeach
                        </select>
                        <div class="invalid-feedback">
                            @error('employer_supplier') {{ $message }} @enderror
                        </div>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" name="employer_rate" id="employerRate"
                                class="form-control @error('employer_rate') is-invalid @enderror"
                                placeholder="0.00"
                                step="0.01"
                                min="0"
                                value="{{ old('employer_rate', $labour->rate) }}">
                        </div>
                        ...
                        <div class="invalid-feedback d-block">
                            @error('employer_rate') {{ $message }} @enderror
                        </div>
                    </div>
